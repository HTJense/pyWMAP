...
class WMAPLike(InstallableLikelihood):
	...
	def setup_for_tt_gibbs(self):
		if self.lowl_max < 2:
			raise LoggedError(self.log, f"lowl_max is set to {self.lowl_max}. The Gibbs sampling requires lowl_max >= 2.")
		
		gibbs_filename = os.path.normpath(os.path.join(self.data_folder, self.gibbs_sigma_filename))
		
		self.log.debug(f"Reading gibbs sigma data from from [{gibbs_filename}].")
		...
		numspec = gibbs_chain.header["NUMSPEC"]
		...
		self.sigmas = sigmas[:, :numchains, self.gibbs_first_iteration-1:self.gibbs_last_iteration:self.gibbs_skip]
		...
		self.log.debug(f"Using Gibbs samples from {self.gibbs_first_iteration} to {self.gibbs_last_iteration} in steps of {self.gibbs_skip}.")
		
		self.log.debug(f"Preparing Blackwell-Rao estimator using:")
	...
	def setup_for_tt_beam_ptsrc(self, lmin, lmax):
		...
		if self.beam_include_beam_modes:
			ifn = os.path.normpath(os.path.join(self.data_folder, self.ifn_beam_modes))
			i, l, x = np.loadtxt(ifn, usecols = (0,1,2), unpack = True)
			i = i.astype(int)
			l = l.astype(int)
			
			l = l[i <= self.n_beam_modes]
			x = x[i <= self.n_beam_modes]
			i = i[i <= self.n_beam_modes] - 1
			
			self.beam_mode[l, i] = x
		
		if self.beam_fixed_fiducial_spectrum:
			ifn = os.path.normpath(os.path.join(self.data_folder, self.ifn_fiducial_cltt))
			l, x = np.loadtxt(ifn, usecols = (0,1), unpack = True)
			l = l.astype(int)
			
			self.fiducial_cltt[l] = x
		
		if self.beam_include_ptsrc_mode:
			ifn = os.path.normpath(os.path.join(self.data_folder, self.ifn_ptsrc_mode))
			l, x = np.loadtxt(ifn, usecols = (0,1), unpack = True)
			l = l.astype(int)
	...
	def setup_for_tt_te_covariance(self):
		"""
		    Get TT/TE/TB diagonals
		    Note that all the files are 2-indexed, but to make it easier on python coding (and to maintain the sanity of
		    whomever needs to write/read this code), the datasets are re-ordered to be 0-indexed.
		"""
		tt_filename = os.path.normpath(os.path.join(self.data_folder, self.tt_filename))
		te_filename = os.path.normpath(os.path.join(self.data_folder, self.te_filename))
		tb_filename = os.path.normpath(os.path.join(self.data_folder, self.tb_filename))
		
		ttoff_filename = os.path.normpath(os.path.join(self.data_folder, self.ttoff_filename))
		teoff_filename = os.path.normpath(os.path.join(self.data_folder, self.teoff_filename))
		
		self.log.debug(f"Reading TT diagonal from [{tt_filename}].")
		
		ls, cltt_dat, ntt, fskytt = np.loadtxt(tt_filename, unpack = True, dtype = float)
		ls = ls.astype(int)
		...
		ls, clte_dat, ntt_te, nee_te, fskyte = np.loadtxt(te_filename, usecols = (0,1,3,4,5), unpack = True, dtype = float)
		ls = ls.astype(int)
		...
		ls, cltb_dat, ntt_tb, nbb_tb, fskytb = np.loadtxt(tb_filename, usecols = (0,1,3,4,5), unpack = True, dtype = float)
		ls = ls.astype(int)
		...
		""" Get TT off-diagonal """
		i, j, epsilon, r_off_tttt = np.loadtxt(ttoff_filename, usecols = (0,1,2,3), unpack = True)
		i = i.astype(int)
		j = j.astype(int)
		...
		""" Get TE off-diagonal """
		i, j, r_off_tete = np.loadtxt(teoff_filename, usecols = (0,1,2), unpack = True)
		i = i.astype(int)
		j = j.astype(int)
	...
	def setup_for_lowl_te_ee_bb(self):
		""" Initialize the TE/EE/BB low-l likelihood. Note that it is not implemented and should be disabled. """
		raise NotImplementedError("The Low-l polarization likelihood is not implemented. Set .use_lowl_pol = False. to disable this component. It may be implemented in the future.")
		...
		self.wl = np.loadtxt(os.path.normpath(os.path.join(self.data_folder, self.TEEEBB_filename_9)), usecols = (0,))
		
		self.ninvplninv2 = np.zeros((self.mp * (2 * self.mp + 1), 2 * self.nlmax))
		
		tmp = fits.open(os.path.normpath(os.path.join(self.data_folder, "lowlP/std", self.TEEEBB_filename_0)))
		...
		for j in np.arange(self.mp * 2):
			for i in np.arange(j + 1):
				...
				self.ninvplninv2[k, 1:self.nlmax-1] = ninvplninv3[2:self.nlmax, ip, jp]
				k += 1
		
		tmp = fits.open(os.path.normpath(os.path.join(self.data_folder, "lowlP/std", self.TEEEBB_filename_1)))
		ninvplninv3 = tmp[0].data.T
		tmp.close()
		...
		for j in np.arange(self.mp * 2):
			for i in np.arange(j + 1):
				ip = self.ngood[ i % self.mp ] + self.np * (i // self.mp)
				jp = self.ngood[ j % self.mp ] + self.np * (j // self.mp)
				self.ninvplninv2[k, self.nlmax:2*self.nlmax-2] = ninvplninv3[2:self.nlmax, ip, jp]
				k += 1
		
		tmp = fits.open(os.path.normpath(os.path.join(self.data_folder, "lowlP/std", self.TEEEBB_filename_2)))
		...
		self.w_r3[ip + self.mp] = T[ self.ngood[ip] ] * self.mask_r3[ self.ngood[ip] ]
		...
		self.Dp0[ip, jp + self.mp] = NinvQUr3[ self.ngood[ip], self.ngood[jp] + self.np ]
		self.Dp0[ip + self.mp, jp] = NinvQUr3[ self.ngood[ip] + self.np, self.ngood[jp] ]
		self.Dp0[ip + self.mp, jp + self.mp] = NinvQUr3[ self.ngood[ip] + self.np, self.ngood[jp] + self.np ]
		
		self.alm_tt = np.zeros((self.nlmax, self.nlmax))
		tmp_alm_tt = np.genfromtxt(os.path.normpath(os.path.join(self.data_folder, self.TEEEBB_filename_5)), converters = { 0 : lambda x: np.complex(*np.safe_eval(x)) })
		self.alm_tt[ np.triu_indices(self.alm_tt.shape[0]) ] = tmp_alm_tt[:self.nlmax * (self.nlmax + 1) // 2]
		
		tmp = fits.open(os.path.normpath(os.path.join(self.data_folder, "lowlP/std", self.TEEEBB_filename_6)))
		ninvy = tmp[0].data.T
	...
	def loglike_lowl_TT(self, cltt):
		"""
			Calculate the low l TT likelihood (with either the gibbs or pixel method).
		"""
	...
	def loglike_lowl_TEEEBB(self, cltt, clte, clee, clbb):
		...
		p_r3[ip] = np.nansum( clte[np.newaxis,ls] / cltt[np.newaxis,ls] * self.wl[np.newaxis,ls] * self.alm_tt[np.newaxis,ls,0] * self.NinvY[self.ngood[ip], 3, np.newaxis], axis = 1)
		p_r3[ip + self.mp] = np.nansum( clte[np.newaxis,ls] / cltt[np.newaxis,ls] * self.wl[np.newaxis,ls] * self.alm_tt[np.newaxis,ls,0] * self.NinvY[self.ngood[ip] + self.np, 3, np.newaxis], axis = 1)
	...
	def loglike_init_covterms(self, cls, TTTT, TTTE, TETE, TTTB, TBTB):
		"""
			Calculate the TTTT/TTTE/TETE/TTTB/TBTB diagonal components of the high l TT/TE/TB likelihood.
		"""
		ls = np.arange(self.ttmin, self.ttmax+1)
		
		if not "tt" in cls: cls["tt"] = np.zeros((self.ttmax+1))
		if not "te" in cls: cls["te"] = np.zeros((self.temax+1))
		if not "tb" in cls: cls["tb"] = np.zeros((self.temax+1))
		if not "ee" in cls: cls["ee"] = np.zeros((self.temax+1))
		if not "bb" in cls: cls["bb"] = np.zeros((self.temax+1))
		
		TTTT[ls] = 2.0 * np.power(cls["tt"][ls] + self.ntt[ls], 2.0) / ((2.0 * ls + 1.0) * np.power(self.fskytt[ls], 2.0))
		
		ls = np.arange(self.temin, self.temax+1)
		
		TETE[ls] = ((cls["tt"][ls] + self.ntt_te[ls]) * (cls["ee"][ls] + self.nee_te[ls]) + np.power(cls["te"][ls], 2.0)) / ((2.0 * ls + 1.0) * np.power(self.fskyte[ls], 2.0))
		TTTE[ls] = 2.0 * (cls["tt"][ls] + self.ntt[ls]) * cls["te"][ls] / ((2.0 * ls + 1.0) * self.fskytt[ls] * self.fskyte[ls])
		
		TBTB[ls] = ((cls["tt"][ls] + self.ntt_tb[ls]) * (cls["bb"][ls] + self.nbb_tb[ls]) + np.power(cls["tb"][ls], 2.0)) / ((2.0 * ls + 1.0) * np.power(self.fskytb[ls], 2.0))
	...
	def get_fisher_TTTT(self, cltt, TTTT, TTTE, TETE):
		...
		diag[ls] = 1.0 / TTTT[ls]
		diag[ls_te] = TETE[ls_te] / (TTTT[ls_te] * TETE[ls_te] - TTTE[ls_te] * TTTE[ls_te])
	...
	def loglike_highl_TT(self, cltt, TTTT, TTTE, TETE):
		...
		off_log_curv[self.ttmin:self.ttmax+1,self.ttmin:self.ttmax+1] = (cltt[np.newaxis,ls] + self.ntt[np.newaxis,ls]) * fisher[ls,:][:,ls] * (cltt[ls,np.newaxis] + self.ntt[ls,np.newaxis])
		...
		logl = 0.0
		logl += (2.0 / 3.0) * ((z - zbar) @ off_log_curv[ls,:][:,ls] @ (z - zbar))
	...
	def loglike_TT_beam_ptsrc(self, cltt, fisher):
		...
		if self.beam_include_beam_modes:
			if self.beam_fixed_fiducial_spectrum:
				mode[ls,:self.n_beam_modes] = self.beam_mode[ls,:] * self.fiducial_cltt[ls, np.newaxis]
			else:
				mode[ls,:self.n_beam_modes] = self.beam_mode[ls,:] * cltt[ls, np.newaxis]
		
		if self.beam_include_ptsrc_mode:
			mode[ls, self.ptsrc_mode_index] = self.ptsrc_err * self.ptsrc_mode[ls]
		...
		a[:self.nmodes] = np.nansum( (self.cltt_dat[ls] - cltt[ls])[...,np.newaxis] * F_mode[ls,:], axis = 0)
	...
	def get_fisher_TETE(self, clte, TTTT, TTTE, TETE):
		...
		diag[ls] = 1.0 / TETE[ls]
		diag[ls_lo] = TTTT[ls_lo] / (TTTT[ls_lo] * TETE[ls_lo] - TTTE[ls_lo] * TTTE[ls_lo])
		np.fill_diagonal(fisher, diag)
	...
	def loglike_highl_TE(self, clte, TTTT, TTTE, TETE):
		...
		_, logdet = np.linalg.slogdet(fisher[self.te_highl_start:,self.te_highl_start:])
	...
	def loglike(self, Cls, **params):
		...
		if self.use_highl_TT:
			components["MASTER_TTTT"] = self.loglike_highl_TT(Cls.get("tt"), TTTT, TTTE, TETE)
		
		if self.use_highl_TE:
			logchi, logdet = self.loglike_highl_TE(Cls.get("te"), TTTT, TTTE, TETE)
			components["MASTER_TETE_chi2"] = logchi
			components["MASTER_TETE_det"] = logdet
		
		if self.use_highl_TB:
			logchi, logdet = self.loglike_highl_TB(Cls.get("tb"), TTTT, TTTB, TBTB)
		...
		if self.use_highl_TT and self.use_highl_TT_beam_ptsrc:
			...
			components["beamptsrc_TT"] = self.loglike_TT_beam_ptsrc(Cls.get("tt"), fisher)
